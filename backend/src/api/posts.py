...
def add_post(
    post: schemas.PostCreate, db: Session = Depends(get_db), current_user: schemas.Post = Depends(get_current_user)
) -> models.Post:
...
def remove_post(
    id: int, db: Session = Depends(get_db), current_user: schemas.Post = Depends(get_current_user)
) -> models.Post:
...
def add_post(
    post_update: schemas.PostUpdate, id: int, db: Session = Depends(get_db), current_user: schemas.Post = Depends(get_current_user)
) -> models.Post:
    post = crud.get_post(db, id)
    if post.user_id != current_user.id:
        return {"message": f"User doesn't have priveleges to delete post with id={id}"}   
...
def get_posts(n_page: int, db: Session = Depends(get_db), current_user = Depends(get_current_user), username_filter: str | None = None):
...
def get_number_of_pages(db: Session = Depends(get_db), current_user = Depends(get_current_user)):
    posts = crud.get_posts(db)
